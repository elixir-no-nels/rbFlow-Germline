...
BWA_IMG = 'Tools.simg'
GATK_IMG = 'Tools.simg'
#GATK_IMG = "ghislain/germlinetest:latest"
#BWA_IMG = "ghislain/germlinetest:latest"
...
## BWA_MEM Mapping with CSV input list
def mapping_sorting_from_list(input_csv: '', output_dir: '', add_suffix: '', reference: {}, config: {}, container: {container: false}, wf_log: false)
  cpu_num = config[:cpu]
  FileUtils.mkdir_p(output_dir)
  Logger.info message: "bwa mem mapping from #{input_csv} list", logfile: wf_log
  File.open(input_csv, "r") do |f|
    f.each do |line|
      ...
      # Mapping
      base_name    = "#{sample}_#{lane}_#{flowcell_id}"
      output_file  = base_name + add_suffix + '_dedup' + '.bam'
      metric_file  = base_name + add_suffix + '_dedup' + '.metric'
      skip_test    = File.exist?("#{output_dir}/#{output_file}")
      step         = Runner.new task_name: "BWA_MEM_map_#{base_name}", wf_log: wf_log, task_log_dir: output_dir
      full_name_R1 = File.expand_path file_name_R1
      full_name_R2 = File.expand_path file_name_R2
      ...
      step.cmd_line << "#{full_name_R1}"
      step.cmd_line << "#{full_name_R2}"
      # The output from BWA is piped in GATK mark duplicate
      ram     = config[:ram]
      ...
      step.cmd_line << "|"
		  step.cmd_line << binary
		  step.cmd_line << 'SortSam'
		  step.cmd_line << "--INPUT=/dev/stdin"
...
## GATK4 merge all input for each sample in a sorted Bam file
def mark_duplicates_merge(input_dir: '', input_filter: '*', output_dir: '', add_suffix: '', reference: {}, group_by_samples: false, config: {}, container: {container: false}, wf_log: false)
  ram            = config[:ram]
  task_name      = "GATK_MergeSortSam"
  group_spliter  = '_' # Split file names on this character
  group_on_index = 0   # Sample are on the first par of the splited file name
...
def gatk_variant_recalibrator_indel(input_dir: '', input_filter: '*', output_dir: '', reference: {}, config: {}, container: {container: false}, wf_log: false)
  ...
  for file in file_list
    ...
    step.cmd_line << '-mode INDEL'
    step.cmd_line << "--tranches-file #{output_dir}/#{tranche_out}"
    step.cmd_line << "--recal_file    #{output_dir}/#{recall_out}"
    step.cmd_line << '--ts_filter_level 95.0 '
...
def gatk_variant_recalibrator_snp(input_dir: '', input_filter: '*', output_dir: '', reference: {}, config: {}, container: {container: false}, wf_log: false)
  ...
  for file in file_list
    ...
    step.cmd_line << "--tranches-file #{output_dir}/#{tranche_out}"
    step.cmd_line << "--recal_file    #{output_dir}/#{recall_out}"
    step.cmd_line << '--ts_filter_level 99.6'
    step.cmd_line << interval_opt
    step.run skip: skip_test, debug: false
  end
  FileUtils.rm_rf(tmp_dir)
...
def gatk_ReadsPipelineSpark(input_dir: '', input_filter: '*', output_dir: '', reference: {}, config: {}, container: {container: false}, wf_log: false)
cpu_num = config[:cpu]
ram     = config[:ram]
tmp_dir = output_dir + '/tmp'
FileUtils.mkdir_p(output_dir)
FileUtils.mkdir_p(tmp_dir)
Logger.info message: 'GATK ReadsPipelineSpark', logfile: wf_log
file_list = get_file_names path: input_dir, filter: '*.bam', full_path: true, wf_log: wf_log
bin_cmd = []
bin_cmd << 'java'
bin_cmd << "-Xmx#{ram}G"
...
for file in file_list
  base_name  = File.basename file, '.bam'
  output_vcf = base_name + '_recalib' + '.g.vcf'
  output_bam = base_name + '_recalib' + '.bam'
  skip_test  = File.exist?("#{output_dir}/#{output_vcf}")
  step       = Runner.new task_name: "GATK_ReadsPipelineSpark_#{base_name}", wf_log: wf_log, task_log_dir: output_dir
  step.in_container(engine: container[:type], imgURL: "#{container[:repo]}#{BWA_IMG}", additional_options: container[:optional_args], mountList: container[:mountList]) if container[:container]
  step.cmd_line << binary
  if config[:spark]
    step.cmd_line << 'ReadsPipelineSpark'
    step.cmd_line << "--spark-master local[#{cpu_num = config[:cpu]}]"
    step.cmd_line << "-R #{reference[:genome_2bit]}"
  else
    Logger.error message: "This tool can run with spark enable only", logfile: wf_log
    exit
  end
  step.cmd_line << "-I #{file}"
  step.cmd_line << "-O #{output_dir}/#{output_vcf}"
  step.cmd_line << "--TMP_DIR #{tmp_dir}"
  step.cmd_line << "--output-bam #{output_dir}/#{output_bam}"
  step.cmd_line << "--emit-ref-confidence GVCF"
  step.cmd_line << "--known-sites #{reference[:kgenomes]}"
  step.cmd_line << "--known-sites #{reference[:mills]} "
  step.cmd_line << "--known-sites #{reference[:dbsnp]} "
...
## GATK4 combineGVCFs
def combineGVCFs(input_dir: '', input_filter: '*', output_dir: '', add_suffix: '', reference: {}, config: {}, container: {container: false}, wf_log: false)
  ram            = config[:ram]
  task_name      = "GATK_combineGVCFs"
  group_spliter  = '_' # Split file names on this character
  group_on_index = 0   # Sample are on the first par of the splited file name
  tmp_dir        = output_dir + '/tmp'
  FileUtils.mkdir_p(output_dir)
  FileUtils.mkdir_p(tmp_dir)
  Logger.info message: 'GATK combineGVCFs', logfile: wf_log
  file_list   = get_file_names path: input_dir, filter: "*#{input_filter}*.g.vcf", full_path: true
  Logger.info message: " inputs : #{file_list.size} files found", logfile: wf_log
  bin_cmd = []
  bin_cmd << 'java'
  bin_cmd << "-Xmx#{ram}G"
  bin_cmd << "-Djava.io.tempdir=#{tmp_dir}"
  ...
  base_name   = 'Merged_gvcfs'
  output_file = base_name + add_suffix + '.g.vcf'
  skip_test   = File.exist?("#{output_dir}/#{output_file}")
  step        = Runner.new task_name: "#{task_name}_#{base_name}", wf_log: wf_log, task_log_dir: output_dir
  step.in_container(engine: container[:type], imgURL: "#{container[:repo]}#{BWA_IMG}", additional_options: container[:optional_args], mountList: container[:mountList]) if container[:container]
  step.cmd_line << binary
  step.cmd_line << 'CombineGVCFs'
  step.cmd_line << inputs.join(' ').to_s
  step.cmd_line << "-O #{output_dir}/#{output_file}"
  step.cmd_line << "-R #{reference[:genome_fas]}"
  #step.cmd_line << "-G Standard -G AS_Standard" # Allele specific
